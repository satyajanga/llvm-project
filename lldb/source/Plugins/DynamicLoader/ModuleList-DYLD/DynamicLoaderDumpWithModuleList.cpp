...
DynamicLoaderDumpWithModuleList::GetLoadedModuleLinkAddr(
...
void DynamicLoaderDumpWithModuleList::DidAttach() {
  ...
  LoadAllModules([&](const std::string &name, addr_t base_addr,
    ...
    if (base_addr == m_vdso_base)
      return;
    module_info_list.emplace_back(
        ModuleInfo{name, base_addr, module_size, link_map_addr});
  });

  // Load modules in parallel or sequentially based on target setting
  auto load_module_fn = [this, &module_list,
                         &log](const ModuleInfo &module_info) {
