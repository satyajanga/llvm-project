...
TEST_F(ObjectContainerClangOffloadBundleTest, LoadsCompressedDeviceImage) {
  ...
  DataBufferSP gpu_data = gpu_file->moduleSpec().GetData();
  llvm::ArrayRef<uint8_t> gpu_bytes(gpu_data->GetBytes(),
  ...
  ASSERT_EQ(2u, ObjectFile::GetModuleSpecifications(file, 0, 0, specs));
  ...
  ASSERT_NE(device_spec.GetData(), nullptr);
